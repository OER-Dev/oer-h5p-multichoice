...
import WavesIcon from './icons/SoundsWaves';
/**
 ...
 * @returns {H5P.OERMultiChoice}
 * @constructor
 */
H5P.OERMultiChoice = function (options, contentId, contentData) {
  if (!(this instanceof H5P.OERMultiChoice))
    return new H5P.OERMultiChoice(options, contentId, contentData);
  var self = this;
  ...
  // If parent exists (must be OER Quiz content type) then 
  if (parent) {
    this.parent = parent;
    parent.on('quizStarted', (e) => {
      self.quizStarted = true;
    });
    parent.on('questionChanged', (question) => {
      if (self.quizStarted) {
        // If the quiz is started and this question is the currently shown and answer has not been given then autoplay sound if checked
        if (self.subContentId === question.data.subContentId && options.audio.autoplay && !self.getAnswerGiven()) {
          setTimeout(playQuestionAudio, 100);
        }
      }
    });
  }
  ...
  var texttemplate =
    '<ul class="h5p-answers h5p-answers--<%= optionStyle %>" role="<%= role %>" aria-labelledby="<%= label %>">' +
    ' <% if (audio && audio.file) { %>' +
    '   <li class="oer-playwrapper"><button type="button" class="oer-playaudio">' + 
    '       <span class="oer-play__icon" aria-hidden="true"></span>' + 
    '       <span class="oer-play__label" >Play sound</span>' +
    '     </button>' +
    '   </li>' +   
    '   <li><audio class="oer-audioplayer" src="<%= audioFile %>"></audio>' +
    ' <% } %>' +
  ...
  /*
    Start OER MultiChoice
    Play audio callback
  */
  var playQuestionAudio = () => {
    var $audioPlayer = $myDom.find('audio')[0];
  ...
  // Audio icon buttons
  var renderPlayIcon = () => {
    var $playButton = $myDom.find('button.oer-playaudio');
    var $iconEl = $myDom.find('span.oer-play__icon');
    $playButton.on('click', playQuestionAudio);
    ReactDOM.render(<PlayIcon />, $iconEl[0]);
  };
  ...
  var renderWavesIcon = () => {
    ...
    var $iconEl = $myDom.find('span.oer-play__icon');
    $playButton.on('click', playQuestionAudio);
    ReactDOM.render(<WavesIcon />, $iconEl[0]);
  }

  var handleAudioFinished = () => {
    setTimeout(renderPlayIcon, 1100);
  }
  // END OER MultiChoice
  ...
  self.registerDomElements = function () {
    ...
    /* Start OER Multichoice handle question settings */
    // If the question or quiz wants to hide the question text then hide it
    if (params.audio.hideQuestionText  || (parent && parent.getQuizConfig().hideQuestionText)) {
      self.setIntroduction('<div id="' + params.label + '" style="height: 0; overflow: hidden; visibility: hidden;">' + params.question + '</div>');
    } else {
      // Register Introduction
      self.setIntroduction('<div id="' + params.label + '">' + params.question + '</div>');
    }
    // If audio file is defined then make sure to get the right path
    if (params.audio && params.audio.file) {
      var re = new RegExp("^(http|https)://", "i");
      var match = re.test(params.audio.file[0].path);
      // This check is necessary due to audio files that have been added via url has an absolute url and those uploaded a relative
      if (match) {
        params.audioFile = params.audio.file[0].path;
      } else {
        params.audioFile = H5P.getContentPath(this.contentId) + '/' + params.audio.file[0].path;
      }
    }
    /* End OER Multichoice handle question settings */
    
    /* Start OER MultiChoice used to have code here for managing image options. Currently not in use */
    // let hasImages = false;
    params.optionStyle = 'square'; // hasImages ? 'images' : 'square'; 

    if (this.parent) { //  && !hasImages
      const parentConfig = this.parent.getQuizConfig();
      params.optionStyle = parentConfig.squareOptions ? 'square' : 'rows';
      if (parentConfig.autoProgression && parentConfig.autoProgression.enabled) {
        params.autoProgression = parentConfig.autoProgression;
      }
    }
    /* End of OER Code */
    ...
    /* Start OER code for managing audio file and autoplay */
    if (params.audio && params.audio.file) {
      renderPlayIcon();
      var $audioPlayer = $myDom.find('audio')[0];
      $audioPlayer.addEventListener('ended', handleAudioFinished);
    }

    // If autoplay try to play it
    if (params.audio && params.audio.autoplay) {
      if (self.parent && self.quizStarted) {
        playQuestionAudio();
      } else if (!self.parent) {
        playQuestionAudio();
      }
    ...
    $answers.click(function () {
      if (parent && parent.getQuizConfig) {
        const config = parent.getQuizConfig();
        if (config.autoProgression && config.autoProgression.enabled) {
          const delay = config.autoProgression.delayTime;
          if (self.autoProgressTimeout) {
            clearTimeout(self.autoProgressTimeout);
          }
          self.autoProgressTimeout = setTimeout(() => self.trigger('autoprogress'), delay);
        }
      }
      toggleCheck($(this));
    }).keydown(function (e) {
  ...
  H5P.OERMultiChoice.counter = (H5P.OERMultiChoice.counter === undefined ? 0 : H5P.OERMultiChoice.counter + 1);
  params.role = (params.behaviour.singleAnswer ? 'radiogroup' : 'group');
  params.label = 'h5p-mcq' + H5P.OERMultiChoice.counter;
...
H5P.OERMultiChoice.prototype = Object.create(H5P.Question.prototype);
H5P.OERMultiChoice.prototype.constructor = H5P.OERMultiChoice;
